...
async function uploadImages(request,env){const form=await request.formData(),versionId=form.get('version_id'),m=await manifestForVersion(env,versionId),refs=collectImageRefs(m.course),allowed=new Map(refs.map(x=>[x.path,x])),files=form.getAll('files');if(!files.length)return json({error:'No files uploaded'},400);const results=[];for(const file of files){if(!(file instanceof File))continue;let path=String(form.get(`path:${file.name}`)||file.name).replace(/^\/+/, '');if(!allowed.has(path)){const matches=refs.filter(x=>x.file_name===file.name||x.path.endsWith('/'+file.name));if(matches.length===1)path=matches[0].path}if(!allowed.has(path)){results.push({file:file.name,ok:false,error:'Filename/path is not referenced by the course'});continue}const ref=allowed.get(path),bytes=await file.arrayBuffer();if(!/\.webp$/i.test(path))path=path.replace(/\.[^.]+$/,'.webp');const hash=await crypto.subtle.digest('SHA-256',bytes),sha=[...new Uint8Array(hash)].map(b=>b.toString(16).padStart(2,'0')).join(''),imageId=ref.image_id||crypto.randomUUID(),createdAt=now(),courseVersion=m.row.version_label||m.row.version_id,fileName=path.split('/').pop();await env.ASSETS.put(path,bytes,{httpMetadata:{contentType:'image/webp'},customMetadata:{image_id:imageId,concept_id:ref.concept_id||'',version_id:versionId,sha256:sha}});await env.DB.prepare(`INSERT OR REPLACE INTO images(image_id,concept_id,file_name,r2_key,width,height,format,file_size,sha256,people_count,contains_text,culturally_specific,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(imageId,ref.concept_id||'',fileName,path,512,512,'webp',bytes.byteLength,sha,0,0,0,createdAt,createdAt).run();await env.DB.prepare(`INSERT OR REPLACE INTO course_card_images(course_id,course_version,lesson_id,activity_id,image_id) VALUES(?,?,?,?,?)`).bind(m.row.course_id,courseVersion,String(ref.item_id||'').split('-').slice(0,2).join('-')||'',ref.item_id||'',imageId).run();results.push({file:file.name,path,ok:true,size:bytes.byteLength,image_id:imageId})}return json({ok:results.some(x=>x.ok),results})}
